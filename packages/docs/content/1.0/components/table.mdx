...
name: Bootstrap React Table
description: Documentation and examples for opt-in styling of Bootstrap React Tables.
menu: Components
...
Due to the widespread use of `<CTable>` elements across third-party widgets like calendars and date pickers, Bootstrap React's tables are **opt-in**. All table styles are not inherited in Bootstrap React, meaning any nested tables can be styled independent from the parent.

Using the most basic table Bootstrap React, here's how `<CTable>`-based tables look in Bootstrap React.
...
[Border color utilities](https://getbootstrap.com/docs/5.1/utilities/borders#border-color) can be added to change colors:
